...
const Work = () => {
    return (
        <div id='mywork' className='min-h-screen bg-soft dark:bg-black pt-10'>
                ...
                <div className='w-full sm:w-full md:w-9/12 mt-5'>
                    {content.work.works.map((data, index) => (
                        <a href={data.url} rel="noopener noreferrer" target="_blank" key={index}
                            className={`relative flex flex-col p-3.5 border-indigo-600 md:border-b-4 ${index % 2 === 0 ? 'border-l-4 md:items-start' : 'md:items-end md:border-r-4 md:text-right'} dark:hover:bg-darkblue hover:bg-white hover:shadow-2xl cursor-pointer anim`}
                        >
                            <p className={`absolute top-0 bg-indigo-600 w-max p-2.5 text-white shadow-2xl text-2xl ${index % 2 === 0 ? 'left-0' : 'md:right-0'}`}>{data.date}</p>
